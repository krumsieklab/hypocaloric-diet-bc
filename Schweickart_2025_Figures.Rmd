...
mets_res_file <- 
  paste0(manuscript_folder,"/Processed_Data_and_Results/known_mets_association_results.xlsx")
...
MWAS_file <- 
  paste0(manuscript_folder,"/Processed_Data_and_Results/NHS2.MWAS.withBMI.csv")
...
#########################################
#### Figure 2 a and c Forest plots ######
########################################

forest_plot <- function(change_df){
  ggplot(change_df, aes(x=LFD_mean, y=Standardized.name))+
  geom_vline(xintercept=0, color="black")+
  geom_linerange(aes(xmin=LFD_lower, xmax=LFD_upper), linewidth = 2, color = magma(10)[5])+
  geom_point( aes(x= LFD_mean, y=Standardized.name),shape=21, size=5, fill = magma(10)[5], color="black") +
  geom_linerange(aes(xmin=KD_lower, xmax=KD_upper),linewidth=2,color = magma(10)[9])+
  geom_point(aes(x=KD_mean, y = Standardized.name), shape=21, size=5, fill = magma(10)[9], color="black") +
...
NHS2_mets_res_all$Standardized.name[NHS2_mets_res_all$Standardized.name=="alpha-hydroxybutyrate/beta-hydroxybutyrate/hydroxyisobutyrate"] <-"2,3-hydroxybutyrate"
...
NHS2_mets_res_all %<>% mutate(Molecular.Annotation = ifelse(grepl("acetyl",metabolite_name), "N-acetyl Metabolites", Molecular.Annotation))

forest_plot(NHS2_mets_res_all %>% 
              filter(Molecular.Annotation %in% c("Amino Acid","N-acetyl Metabolites")) %>% 
              filter(KD_change_p_adj < 0.05  ||
...
ggplot(trigs_plot_df, aes(x=length, y = saturation, fill = estimate, size = -log10(p_adj), color = estimate, shape = estimate < 0))+
  geom_point()+
  theme_minimal() + 
  labs( color="Fold Change", shape = "Direction of change")+
  scale_fill_viridis(option = "plasma")+
  scale_color_viridis(option = "plasma",breaks = c(-1.5, 0, 1.5))+
  scale_shape_manual(values=c(24,25), name= "Direction of change",labels = c("Increased Abundance", "Decreased Abundance"))+
...
fig_2a_plot_df$Standardized.name[fig_2a_plot_df$Metabolite.x=="alpha-hydroxybutyrate/beta-hydroxybutyrate/hydroxyisobutyrate"] <-"2,3-hydroxybutyrate"

fig_2a_plot_df$Metabolite.x[fig_2a_plot_df$Metabolite.x=="PC(P-36:4)/PC(O-36:5)_B"] <-"PC(P-36:4)/PC(O-36:5)"

kd_mets_to_label <- c("DG 36:2","TG 54:2", "DG 38:5", "Acetaminophen", "CE 18:3", "CE 20:3", "NA-Gly 18:1(9Z)",  "2,3-hydroxybutyrate", "CAR 4:0;OH")

fig_2a_plot_df %<>% mutate(label = ifelse(Standardized.name %in% kd_mets_to_label, Standardized.name, "")) 
...
lfd_mets_to_label <- c( "PE 40:6","PE 38:6", "Acetaminophen", "NA-Gly 18:1(9Z)",   "N1-Acetylspermidine","CE 18:3","CE 18:0")

fig_2b_plot_df %<>% mutate(label = ifelse(Standardized.name %in% lfd_mets_to_label, Standardized.name, "")) 
...
top_positive_concordance_mets_kd <- fig_2a_plot_df %>% 
  select(NHSII_concordance, Standardized.name) %>% 
  arrange(desc(NHSII_concordance)) %>% 
  .[1:4,] %>% .$Standardized.name %>% 
  unique() %>% 
  rev()

fig2c_kd_top_pos_plot_df<- fig_2a_plot_df %>% 
  select(NHSII_concordance, fc_scaled, Standardized.name) %>% 
  melt(id.vars =c("Standardized.name")) %>% 
  filter(Standardized.name %in% top_positive_concordance_mets_kd) %>% 
  mutate(variable = ifelse(variable == "NHSII_concordance", "NHSII Concordance", "Scaled Fold Change"))

fig2c_kd_top_pos_plot_df$Standardized.name <- 
  factor(fig2c_kd_top_pos_plot_df$Standardized.name, 
         levels = top_positive_concordance_mets_kd)

fig2c_kd_top_pos_plot <- ggplot(fig2c_kd_top_pos_plot_df,
                                aes(x = value, y = Standardized.name, color = variable)) +
  geom_point(size =5) +
...
top_negative_concordance_mets_kd <- fig_2a_plot_df %>% 
  ...
  .[1:8,] %>% .$Standardized.name %>% 
  unique()

fig2c_kd_top_neg_plot_df<- fig_2a_plot_df %>% 
  select(NHSII_concordance, fc_scaled, Standardized.name) %>% 
  melt(id.vars =c("Standardized.name")) %>% 
  filter(Standardized.name %in% top_negative_concordance_mets_kd) %>% 
  mutate(variable = ifelse(variable == "NHSII_concordance", "NHSII Concordance", "Scaled Fold Change"))

fig2c_kd_top_neg_plot_df$Standardized.name <- 
  factor(fig2c_kd_top_neg_plot_df$Standardized.name, 
...
fig2c_kd_diff_plot_df<- fig_2a_plot_df %>% 
  select(NHSII_concordance, fc_scaled, Standardized.name) %>% 
  melt(id.vars =c("Standardized.name")) %>% 
  filter(Standardized.name %in% different_concordance_mets_kd) %>% 
  mutate(variable = ifelse(variable == "NHSII_concordance", "NHSII Concordance", "Scaled Fold Change"))

fig2c_kd_diff_plot_df$Standardized.name <- 
  factor(fig2c_kd_diff_plot_df$Standardized.name, 
         levels = different_concordance_mets_kd)


fig2c_kd_diff_plot <- ggplot(fig2c_kd_diff_plot_df, aes(x = value, y =Standardized.name, color = variable)) +
  geom_point(size =5) +
  scale_color_manual(values = palette[c(3,7)])+
  labs(
...
fig2d_lfd_top_pos_plot_df<- fig_2b_plot_df %>% 
  select(NHSII_concordance, fc_scaled, Standardized.name) %>% 
  melt(id.vars =c("Standardized.name")) %>% 
  filter(Standardized.name %in% top_positive_concordance_mets_lfd) %>% 
  mutate(variable = ifelse(variable == "NHSII_concordance", "NHSII Concordance", "Scaled Fold Change"))

fig2d_lfd_top_pos_plot_df$Standardized.name <- 
  factor(fig2d_lfd_top_pos_plot_df$Standardized.name, 
...
top_negative_concordance_mets_lfd <- fig_2b_plot_df %>% 
  select(NHSII_concordance, Standardized.name) %>% 
  arrange(NHSII_concordance) %>% 
  .[1:9,] %>% .$Standardized.name %>% 
  unique()

fig2d_lfd_top_neg_plot_df<- fig_2b_plot_df %>% 
  select(NHSII_concordance, fc_scaled, Standardized.name) %>% 
  melt(id.vars =c("Standardized.name")) %>% 
  filter(Standardized.name %in% top_negative_concordance_mets_lfd) %>% 
  mutate(variable = ifelse(variable == "NHSII_concordance", "NHSII Concordance", "Scaled Fold Change"))

fig2d_lfd_top_neg_plot_df$Standardized.name <- 
  factor(fig2d_lfd_top_neg_plot_df$Standardized.name, 
...
fig2d_lfd_diff_plot_df<- fig_2b_plot_df %>% 
  select(NHSII_concordance, fc_scaled, Standardized.name) %>% 
  melt(id.vars =c("Standardized.name")) %>% 
  filter(Standardized.name %in% different_concordance_mets_lfd) %>% 
  mutate(variable = ifelse(variable == "NHSII_concordance", "NHSII Concordance", "Scaled Fold Change"))

fig2d_lfd_diff_plot_df$Standardized.name <- 
  factor(fig2d_lfd_diff_plot_df$Standardized.name, 
         levels = different_concordance_mets_lfd)


fig2d_lfd_diff_plot <- ggplot(fig2d_lfd_diff_plot_df, aes(x = value, y =Standardized.name, color = variable)) +
  geom_point(size =5) +
  scale_color_manual(values = palette[c(3,7)])+
  labs(
    title = "Metabolites with discordant effects",
...
fig3_kd_mets_to_label <- c("TG 46:1", "NA-Gly 18:1(9Z)",  "LPC 20:1", "CE 22:6",  "CAR 4:0;OH")

fig3_kd_plot_df %<>% mutate(label = ifelse(Standardized.name %in% fig3_kd_mets_to_label, Standardized.name, "")) 
...
fig3a_kd <- ggplot(fig3_kd_plot_df, aes(x=Metabolite_FC, y = coef, fill = fc_fill, label = label)) +
  ...
  annotate(geom="text", y = 0.17,x = -0.7, size=8,
           ...
           "(", round(cor.test(fig3_kd_plot_df$Metabolite_FC,
                                        fig3_kd_plot_df$coef)$conf.int[1],3),
           ", ", round(cor.test(fig3_kd_plot_df$Metabolite_FC,
                                        fig3_kd_plot_df$coef)$conf.int[2],3),
           ")"))+
  theme_minimal()+
...
fig3_lfd_mets_to_label <- c("Biliverdin", "N1-Acetylspermidine", "NA-Gly 18:1(9Z)", "CE 22:6",  "CAR 4:0;OH", "Piperine", "Cotinine","TG 52:1","TG 52:0", "TG 50:0")

fig3_lfd_plot_df %<>% mutate(label = ifelse(Standardized.name %in% fig3_lfd_mets_to_label, Standardized.name, "")) 
...
fig3a_lfd <- ggplot(fig3_lfd_plot_df, aes(x=Metabolite_FC, y = coef, fill=fc_fill, label = label)) +
  ...
  annotate(geom="text", y = 0.17,x = -0.7, size=8,
           ...
           "(", round(cor.test(fig3_lfd_plot_df$Metabolite_FC,
                                        fig3_lfd_plot_df$coef)$conf.int[1],3),
           ", ", round(cor.test(fig3_lfd_plot_df$Metabolite_FC,
                                        fig3_lfd_plot_df$coef)$conf.int[2],3),
           ")"))+
  scale_fill_viridis_c(option = "plasma")+
  geom_label_repel(color = "black", fill = "white",size=6,
                   min.segment.length = unit(0, 'lines')) +
